  ...
  mutate(perc = perc*100) %>% 
    ...
    map(~ {
        ...
        hchart(.x, type = "bar", hcaes(x = value, y = perc), color = the_color) %>%
            hc_plotOptions(bar = list(horizontal = TRUE, stacking = "normal")) %>%
            hc_yAxis(title = list(text = "% Party Budget spent on targeting method (in Euro)"))%>%
            hc_xAxis(title = "", labels = list(style = list(fontSize = "10px"))) %>%
            hc_add_theme(hc_theme_google()) %>%
           highcharter::hc_tooltip(
...
contested_dat <- interest_targeting %>% 
  ...
  total_spending_dat <- contested_dat %>% 
    drop_na(value) %>%
    distinct(value, total_spenderino) %>% 
    mutate(value2 = factor(value, the_order)) %>% 
    arrange(value)
  
  annotations <- lapply(1:nrow(total_spending_dat), function(i) {
  list(
    point = list(x = i - 1, y = 1, xAxis = 0, yAxis = 0),
    text = paste0("Total Spend on Audience: ",  currency_symbol, scales::comma(round(total_spending_dat$total_spenderino[i]))),
          # align = "right",
  ...
  hc_annotations(list(labels = annotations)) %>%
    hc_colors(color_dat %>% filter(party %in% contested_dat$party) %>%  arrange(party) %>% pull(colors)) 
}


  
get_contested_graph(election_dat30)
```
