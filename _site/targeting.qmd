...
gg %>%
  ...
  highcharter::hc_yAxis(title = list(text = "\nBudget spent on targeting method (% of Total spend)")) %>%
...
```{r, fig.width=12, fig.height=14, dpi=300}
...
plot_geography <- function(x) {
  ...
  charts <- gg %>%
    split(.$party) %>%
    map(~ hchart(.x, type = "bar", hcaes(x = target, y = perc)) %>%
            hc_plotOptions(bar = list(horizontal = TRUE, stacking = "normal")) %>%
            hc_yAxis(title = list(text = "\nBudget spent on targeting method (Count)")) %>%
            hc_xAxis(labels = list(style = list(fontSize = "10px"))) %>%
            hc_add_theme(hc_theme_google()) %>%
            hc_tooltip(
              formatter = JS("function() {
                return '<b>' + this.point.y + '</b><br>Count: ' + this.point.x;
              }")
            ) %>%
            ...
            hc_chart(width = 600, height = 600)  # Adjust the dimensions as per your needs
    )
